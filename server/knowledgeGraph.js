...
export function buildKnowledgeGraph(db,projectId){
  ...
  for(const revision of (db.bomRevisions||[]).filter(x=>x.projectId===projectId)){add(node(revision.id,'bom_revision',`BOM Rev ${revision.revision}`,revision));edges.push(edge(projectId,revision.id,'has_bom_revision'));if(revision.parentId)edges.push(edge(revision.parentId,revision.id,'derived_into',{method:'bom_revision'}));for(const part of (db.bom||[]).filter(x=>x.projectId===projectId&&x.revision===revision.revision)){const id=`${revision.id}:${part.partNo}`;add(node(id,'bom_item',part.description,part));edges.push(edge(revision.id,id,'contains_part'));for(const supplierId of part.supplierIds||[])edges.push(edge(id,supplierId,'sourced_from'))}}
